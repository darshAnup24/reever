...
Point Reever at a task — *"fix the failing test in `src/foo`"* — and it searches the
...
[pi.dev](https://pi.dev), and [Claude Code](https://claude.ai/code) — Reever makes
different trade-offs that pay off in cost, safety, and observability.

### BM25 tool pre-filtering per turn

Every turn, Reever runs a **BM25 retrieval pass** (via [Ratel](https://ratel.sh)) over
...
built-in. Reever's **shadow-git snapshots** fire before every tool execution —
...
pi.dev and opencode route all work through a single model on every turn. Reever's
...
community plugin. Reever ships **OTLP export natively** — every turn produces a trace
...
backend. Claude Code also ships OTLP natively; the difference is that Reever's local
metrics (`~/.reever/metrics.jsonl`) are always on with no environment variable
required, and content capture defaults are the same: off unless you opt in.

### Offline demo + EU provider

`reever --faux` runs the full TUI with scripted responses — no API key, no network.
None of the three competitors have an equivalent. Reever also ships **Regolo** as a
...
- **Autonomous, with a seatbelt** — Reever drives the tools itself, but stops for
...
- **Lean prompts by default** — broad reads and discovery output are capped,
  while BM25 supplies task-relevant tool schemas instead of resending every
  tool on every agent round. All tools remain reachable through capability search.
...
- **Bring your own model** — local Ollama, OpenRouter, Anthropic, OpenAI, and
  EU-hosted Regolo ship today; switch provider or model mid-session with `/providers` and `/model`.
...
- **Never loses the thread** — sessions are resumable append-only logs, context
  ...
  automatically.
- **Try it with no API key** — `reever --faux` runs the whole TUI offline with
  scripted responses.
...
cd coding_agent
./install.sh        # installs Bun if needed, installs deps, builds, links `reever`
reever                # start the interactive agent
```
...
```bash
reever --faux         # fully offline, scripted demo
```

### Run with a free local model

Reever includes Ollama as a built-in provider, so normal agent runs can stay on
your machine with no API key or usage bill. After installing Ollama and pulling
a model:
...
Then select `/providers ollama` in the TUI. Reever discovers models already
pulled into Ollama; `/model` also includes practical defaults. It uses
`http://127.0.0.1:11434` by default, respects `OLLAMA_HOST`, and supports a
custom endpoint through `/providers configure ollama`. Ollama is only used when
you explicitly select it or include it in a routing profile; it is never chosen
automatically as a fallback for a missing cloud-provider key.

> **Requirements:** [Bun](https://bun.sh) ≥ 1.1 (the TUI uses Bun's FFI and a
> preload transform — plain `node` won't work), a real TTY, and an
> [OpenRouter](https://openrouter.ai/keys) key, another cloud provider, or a
> running Ollama server for real use.
...
<p align="center">
  <img src="docs/media/session.png" alt="Reever handling a task: a read tool call followed by the answer" width="760">
  <br>
  <em>One turn of the loop: Reever calls the <code>read</code> tool, then answers — streamed live.</em>
</p>
...
```bash
reever                                     # interactive TUI
reever "fix the failing test in src/foo"   # interactive, with an opening message
reever --resume <id>                       # resume a saved session (alias: -r)
reever --continue                          # resume this project's latest session (alias: -c)
reever --list-sessions                     # list saved sessions (alias: -l)
reever --plan                              # start in read-only plan mode
reever --auto-accept                       # start in allow-all mode
reever --headless <prompt>                 # run one task to completion, stream to stdout, exit
reever --chat <prompt>                     # single non-agentic completion
reever --faux                              # offline demo, no API key
```
...
| `/commands` | List reusable global and project prompt commands |
| `/instructions` | Inspect active `AGENTS.md` files and precedence |
...
| :---: | :---: |
| ![Reever command palette](docs/media/command-palette.png) | ![Reever model picker](docs/media/model-picker.png) |

### Reusable prompt commands

Put Markdown prompts in `.reever/commands/` for a project or
`~/.reever/commands/` for commands available everywhere. Project commands
override global commands with the same name, and directories create namespaces:

```markdown
<!-- .reever/commands/git/review.md → /git:review -->
---
description: Review a file or change
---
Review {{args}} for correctness, regressions, and missing tests.
```

Run it with `/git:review src/provider/router.ts`. Arguments replace every
`{{args}}` marker, or are appended when the marker is absent. Command files only
expand into model prompts; they never execute embedded shell syntax.

## Configuration

All settings live in **`~/.reever/config.json`**, merged on top of built-in defaults.
...
checkpoints all live under `~/.reever/` and survive upgrades.
...
Reever also records **cost and token metrics** locally (`~/.reever/metrics.jsonl`) by
...
append as JSON lines to `~/.reever/metrics.jsonl` and mirror into the session log.
...
| Metrics file path | `telemetry.metricsFile` | Default `~/.reever/metrics.jsonl`. |

#### OTLP trace export

Reever can export **OTLP traces** following the OpenTelemetry
...
Reever session share a `session.id` so backends like Langfuse group them together.
...
| Service name | `telemetry.otel.serviceName` | Resource `service.name` (default `reever`). |
...
Example — export to Langfuse via `~/.reever/config.json`:
...
`~/.reever/config.json`, or from the TUI via `/settings telemetry on` (also toggleable in the
...
The design and phased build plan live in [`SPEC.md`](./SPEC.md). Reever is built on
...
dependencies, rebuild, and re-link the `reever` command.
