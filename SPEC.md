# SPEC: Build a Terminal Coding Agent ("Reever")
...
**Further reading (patterns study, not primary references):** **Cline** and **Kilo Code** (Roo lineage) — see [`docs/reference/cline-kilo.md`](docs/reference/cline-kilo.md) for patterns mapped to these phases (robust diff editing → Phase 6, orchestrator/subtasks → Phase 8, shadow-git checkpoints, context condensing → Phase 7). **Charm Crush** — see [`docs/reference/crush.md`](docs/reference/crush.md) for LSP integration, hooks/permissions UX, client/server headless embedding, and TUI patterns (issue #327). **Command Code** — see [`docs/reference/command-code.md`](docs/reference/command-code.md) for its Taste learning model and a review-gated preference-ledger design for Reever (issue #413).
...
- `/providers` lists and switches the active provider at runtime, persisting `provider.active` to `~/.reever/config.json`. Because models resolve through the registry on each turn, a switch takes effect on the next turn with no rewiring.
...
export async function connectServer(s: McpServerConfig) {
  const client = new Client({ name: "reever", version: "0.1.0" });
...
**Emitter + exporters** (`src/telemetry/`): a `MetricEvent` union into pluggable sinks. Default = JSONL at `~/.reever/metrics.jsonl`; optional stdout; optional OTel/HTTP later.
```ts
type MetricEvent =
