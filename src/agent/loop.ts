...
import { recordTokensSaved, estimateTokensRemovedByPruning } from "../telemetry/tokens-saved.js";
...
async function resolvePromptContext(
  ...
  ratelResolution: ReturnType<NonNullable<ReeverRatelBundle["resolveToolsForTurn"]>> | undefined;
}> {
...
const compactionOptions = {
    ...
    if (compactionNeeded) {
      ...
      // Record the estimated tokens compaction removed from the next turn — the
      // documented "tokens saved" baseline (see telemetry/tokens-saved.ts).
      recordTokensSaved(estimateTokensRemovedByPruning(preCompact, ctx.messages));
