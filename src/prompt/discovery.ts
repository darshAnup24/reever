...
export function discoverAgentsFiles(cwd: string): DiscoveredFile[] {
  ...
  // The upward walk finds leaf → root. Reverse it before adding global rules so
  // precedence is global → repository root → deepest applicable directory.
  found.reverse();
  const globalPath = join(homedir(), ".reever", "AGENTS.md");
