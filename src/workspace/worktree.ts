...
export interface CreateWorktreeOptions {
  /** Branch name prefix, e.g. `reever/subagent` or `reever/session`. Default: `reever/subagent`. */
  branchPrefix?: string;
...
export function resolveParallelWorktreeBase(parentCwd: string): ParallelWorktreeBaseResult {
  ...
  const snapshot = git(parentCwd, [
    "-c", "user.name=reever",
    "-c", "user.email=reever@localhost",
    "commit-tree", tree.stdout,
    "-p", head.stdout,
    "-m", "reever parallel base snapshot",
  ]);
  git(parentCwd, ["reset", "--mixed", "HEAD"]);
...
export function createWorktree(
  ...
  const branchPrefix = opts.branchPrefix ?? "reever/subagent";
  ...
  if (!dir) {
    const base = mkdtempSync(join(tmpdir(), `reever-wt-${shortId}-`));
    dir = join(base, "tree");
    ownsTempBase = true;
  } else {
