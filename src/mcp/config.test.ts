...
describe("loadMcpConfig", () => {
  ...
  beforeEach(() => {
    ...
    home = mkdtempSync(join(tmpdir(), "reever-mcp-config-"));
  ...
  it("returns empty config and warns on invalid JSON", async () => {
    mkdirSync(join(home, ".reever"), { recursive: true });
    writeFileSync(join(home, ".reever", "mcp.json"), "{ not json");
    const { loadMcpConfig } = await import("./config.js");
    const { config, warnings } = loadMcpConfig();
    expect(config.servers).toEqual({});
  ...
  it("auto-repairs npx -y server placeholder to the filesystem MCP package", async () => {
    mkdirSync(join(home, ".reever"), { recursive: true });
    const path = join(home, ".reever", "mcp.json");
    writeFileSync(
      path,
      JSON.stringify({
        servers: {
  ...
  describe("env var expansion", () => {
    ...
    it("expands ${env:VAR} in command, args, env, and url at load time", async () => {
      ...
      writeFileSync(
        ...
        JSON.stringify({
          servers: {
            fs: {
              type: "stdio",
              command: "${env:REEVER_TEST_BIN}",
              args: ["--root=${env:REEVER_TEST_ROOT}", "."],
              env: { API_KEY: "${env:REEVER_TEST_KEY}" },
            },
            http: {
              type: "http",
              url: "https://${env:REEVER_TEST_HOST}/mcp",
              headers: { Authorization: "Bearer ${env:REEVER_TEST_KEY}" },
            },
          },
        }),
      );
      ...
      const { config, warnings } = loadMcpConfig(
        ...
        withEnv({
          REEVER_TEST_BIN: "npx",
          REEVER_TEST_ROOT: "/tmp",
          REEVER_TEST_KEY: "secret-token",
          REEVER_TEST_HOST: "api.example.com",
        }),
      );
      expect(warnings).toEqual([]);
        ...
        JSON.stringify({
          servers: {
            http: {
              ...
              headers: { Authorization: "Bearer ${REEVER_TEST_CC_TOKEN}" },
            },
          },
        }),
      );
      const { loadMcpConfig } = await import("./config.js");
      const { config } = loadMcpConfig(undefined, withEnv({ REEVER_TEST_CC_TOKEN: "cc-secret" }));
      ...
      writeFileSync(
        ...
        JSON.stringify({
          servers: {
            http: {
              type: "http",
              url: "${env:REEVER_TEST_MISSING_URL:-https://fallback.example.com/mcp}",
            },
          },
        }),
      );
    ...
    it("skips a server and warns when a required env var is missing", async () => {
        ...
        JSON.stringify({
          servers: {
            github: {
              ...
              headers: { Authorization: "Bearer ${env:REEVER_TEST_ABSENT_TOKEN}" },
            },
      ...
      expect(
        warnings.some(
          (w) => w.includes('MCP server "github"') && w.includes("${REEVER_TEST_ABSENT_TOKEN}"),
        ),
      ).toBe(true);
    });
    ...
    it("skips a server when the expanded URL is not a valid URL", async () => {
      ...
      writeFileSync(
        ...
        JSON.stringify({
          servers: {
            bad: { type: "http", url: "${env:REEVER_TEST_HOST}" },
          },
        }),
      );
      const { loadMcpConfig } = await import("./config.js");
      const { config, warnings } = loadMcpConfig(
        undefined,
        withEnv({ REEVER_TEST_HOST: "not a url at all" }),
      );
        ...
        JSON.stringify({
          servers: {
            http: {
              type: "http",
              url: "https://api.example.com/mcp",
              headers: { Authorization: "Bearer ${env:REEVER_TEST_PERSIST_TOKEN}" },
            },
          },
        }),
      );
      const { loadMcpConfig } = await import("./config.js");
      loadMcpConfig(undefined, withEnv({ REEVER_TEST_PERSIST_TOKEN: "do-not-write-me" }));
      const onDisk = readFileSync(path, "utf8");
      expect(onDisk).toContain("${env:REEVER_TEST_PERSIST_TOKEN}");
...
describe("loadMcpConfig – project merge", () => {
  ...
  beforeEach(() => {
    prevHome = process.env.HOME;
    home = mkdtempSync(join(tmpdir(), "reever-mcp-merge-home-"));
    projectDir = mkdtempSync(join(tmpdir(), "reever-mcp-merge-proj-"));
    process.env.HOME = home;
  });
