...
import { createReeverMcpOAuthProvider } from "./oauth-provider.js";

export interface AuthenticateMcpServerOptions {
  /** Authorization code from redirect (for `reever mcp auth <name> --code …`). */
...
async function connectWithOAuthTransport(
  ...
  const provider = createReeverMcpOAuthProvider({
    serverName: name,
    config,
    redirectUrl,
    openBrowser,
