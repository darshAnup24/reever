...
export interface ReeverMcpOAuthProviderOptions {
  serverName: string;
...
export class ReeverMcpOAuthProvider implements OAuthClientProvider {
  private readonly oauthOptions;
  private readonly shouldOpenBrowser;
  private readonly onAuthorizationUrl?: (url: URL) => void;

  constructor(private readonly opts: ReeverMcpOAuthProviderOptions) {
...
export function createReeverMcpOAuthProvider(
  opts: ReeverMcpOAuthProviderOptions,
): ReeverMcpOAuthProvider {
  return new ReeverMcpOAuthProvider(opts);
}
