...
describe("SymbolService cache persistence", () => {
  ...
  beforeEach(async () => {
    ...
    home = await mkdtemp(join(tmpdir(), "reever-symbols-cache-home-"));
    process.env.HOME = home;
    cwd = await mkdtemp(join(tmpdir(), "reever-symbols-cache-"));
  ...
  it("falls back to full warm when cache is corrupt", async () => {
    const workspace = createLocalWorkspace();
    await mkdir(join(home, ".reever", "index"), { recursive: true });
