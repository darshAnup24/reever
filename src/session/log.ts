...
export function sessionsDir(): string {
  return join(homedir(), ".reever", "sessions");
}
...
/** Most recently active session belonging to this project checkout. */
export function latestSessionForCwd(hostCwd: string, scanDir?: string): SessionSummary | undefined {
  return listSessions(scanDir)
    .filter((session) => (session.hostCwd ?? session.cwd) === hostCwd)
    .sort((a, b) => {
      const aTime = Date.parse(a.lastTs || a.createdAt) || 0;
      const bTime = Date.parse(b.lastTs || b.createdAt) || 0;
      return bTime - aTime;
    })[0];
}
