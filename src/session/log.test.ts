...
describe("latestSessionForCwd", () => {
  it("returns the most recently active session for the requested project only", () => {
    const sessionsPath = join(tmpDir, "sessions-latest");
    mkdirSync(sessionsPath, { recursive: true });
    writeFileSync(
      join(sessionsPath, "older.jsonl"),
      '{"type":"session_meta","ts":"2026-01-01T00:00:00.000Z","sessionId":"older","cwd":"/projects/foo","model":"m"}\n'
        + '{"type":"user_message","ts":"2026-01-01T01:00:00.000Z","content":[]}\n',
    );
    writeFileSync(
      join(sessionsPath, "newer.jsonl"),
      '{"type":"session_meta","ts":"2026-01-01T00:00:00.000Z","sessionId":"newer","cwd":"/tmp/worktree","hostCwd":"/projects/foo","model":"m"}\n'
        + '{"type":"user_message","ts":"2026-01-02T01:00:00.000Z","content":[]}\n',
    );
    writeFileSync(
      join(sessionsPath, "other.jsonl"),
      '{"type":"session_meta","ts":"2026-01-03T00:00:00.000Z","sessionId":"other","cwd":"/projects/bar","model":"m"}\n',
    );

    expect(latestSessionForCwd("/projects/foo", sessionsPath)?.sessionId).toBe("newer");
    expect(latestSessionForCwd("/projects/missing", sessionsPath)).toBeUndefined();
  });
});
