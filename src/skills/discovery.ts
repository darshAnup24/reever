 ...
 *   1. All `.reever/skills/` dirs from cwd up to repo root (nearest cwd wins within this tier)
 *   2. All `.claude/skills/` dirs from cwd up to repo root (cross-agent compat)
 *   3. ~/.reever/skills/, ~/.claude/skills/, ~/.agents/skills/ (user-global fallbacks)
 */
export function discoverSkills(cwd: string): SkillMeta[] {
  ...
  // Tier 1: project-local .reever (all ancestor levels before any .claude scan)
  walkSkillRoots(".reever", "skills");
...
export function globalSkillsDir(): string {
  return join(homedir(), ".reever", "skills");
}

/**
 * Classify where a skill lives, for display in the TUI palette.
 *
 *   - `global`  — anything under the user's home dir (`~/.reever`, `~/.claude`, `~/.agents`)
 *   - `claude`  — a project-local `.claude/skills/` (cross-agent compat tier)
 *   - `project` — a project-local `.reever/skills/`
 *
