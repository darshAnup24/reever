...
import { discoverCustomCommands, expandCustomCommand } from "../prompt/custom-commands.js";
...
const HELP_LINES = [
  ...
  "/providers configure [id]     set API keys / provider settings in ~/.reever/config.json",
  ...
  "/commands                     list reusable project/global prompt commands",
  "/instructions                 show the exact AGENTS.md context and precedence",
...
export function processCommand(raw: string, ctx: CommandContext): CommandResult {
  ...
  switch (name) {
    ...
    case "/commands": {
      const commands = discoverCustomCommands(ctx.liveCwd ?? process.cwd());
      if (commands.length === 0) {
        return {
          type: "info",
          message: "No custom commands. Add Markdown prompts under .reever/commands/ or ~/.reever/commands/.",
        };
      }
      return {
        type: "info",
    ...
    case "/instructions": {
      const files = discoverAgentsFiles(ctx.liveCwd ?? process.cwd());
      if (files.length === 0) {
        return { type: "info", message: "No AGENTS.md instructions are active." };
      }
      return {
        type: "info",
        message: "Active instructions (later files take precedence):\n\n"
    ...
    default: {
      ...
      const command = discoverCustomCommands(ctx.liveCwd ?? process.cwd())
        .find((candidate) => candidate.name.toLowerCase() === commandName);
      if (command) return { type: "prompt", prompt: expandCustomCommand(command, arg) };
