...
const readActiveRuns = (home: string) =>
  JSON.parse(readFileSync(join(home, ".reever", "active-runs.json"), "utf8")) as Array<{
    pid: number;
    sessionId: string;
  }>;
...
describe("installCrashDiagnostics", () => {
  ...
  beforeEach(() => {
    ...
    home = mkdtempSync(join(tmpdir(), "reever-crash-test-"));
  ...
  it("flags a prior run whose process is gone and never cleared itself", () => {
    ...
    writeFileSync(
      join(home, ".reever", "active-runs.json"),
      JSON.stringify([
