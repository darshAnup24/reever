...
import { discoverCustomCommands } from "../prompt/custom-commands.js";
...
const SLASH_COMMANDS = [
  ...
  { name: "commands",  label: "/commands",  description: "list reusable prompt commands" },
  { name: "instructions", label: "/instructions", description: "show active AGENTS.md context" },
...
export function App(props: {
  ...
  const performCopy = async (text: string | null | undefined) => {
    if (!text) {
      props.controller.setStatusHint("Nothing to copy — see ~/.reever/sessions/*.jsonl");
      return;
    }
  ...
  const customCommands = createMemo(() => discoverCustomCommands(state().meta.cwd));
  ...
  const filteredCommands = () => {
    ...
    if (!input.startsWith("/")) return available;
    const filter = input.slice(1).toLowerCase();
    if (!filter) return available;
    return available.filter((c) => c.name.startsWith(filter));
  };
  ...
  const openSkillsPalette = () => {
    ...
    if (skills.length === 0) {
      setPalette(null);
      props.controller.setStatusHint(
        "No skills found — add SKILL.md under .reever/skills/ or ask the agent to create one",
      );
      return;
    }
  ...
  const applyCommandResult = (result: CommandResult) => {
    switch (result.type) {
      ...
      case "prompt": {
        void runUserTurn(result.prompt);
        return;
      }
      case "checkpoints": {
  ...
  const handlePaletteSelect = () => {
    ...
    if (p.phase === "commands") {
      ...
      if ("customPrompt" in cmd) {
        closePalette();
        void runUserTurn(cmd.customPrompt);
        return;
      }
      ...
      } else if (name === "commands" || name === "instructions") {
        applyCommandResult(processCommand(`/${name}`, commandContext()));
      }
      return;
    }
      ...
      <box flexShrink={0} flexDirection="column" marginTop={1} paddingTop={1} border={["top"]} borderColor={theme.border}>
            ...
            >
              ...
              <text fg={theme.secondary}>
                {prompt().fields[prompt().fieldIndex]?.secret
                  ? "Paste your API key (saved to ~/.reever/config.json, not sent to the agent)"
                  : "Enter the value below (saved to ~/.reever/config.json)"}
              </text>
            </box>
          )}
        </Show>
        ...
        <Show when={e2bPrompt()}>
          ...
          >
            ...
            <text fg={theme.secondary}>
              Paste your E2B API key (saved to ~/.reever/config.json, enables the task tool)
            </text>
          </box>
        </Show>
        ...
        <Show when={exaPrompt()}>
          ...
          >
            ...
            <text fg={theme.secondary}>
              Paste your Exa API key (saved to ~/.reever/config.json, enables the web_search tool)
            </text>
          </box>
        </Show>
