 ...
 *
 * Reever's renderer runs on Bun's FFI plus a native (Zig) layer. When that layer
 ...
 *     structured entry to `~/.reever/crash.log`. (OpenTUI installs its own
...
function reeverDir(): string {
  return join(homedir(), ".reever");
}

export function crashLogPath(): string {
  return join(reeverDir(), "crash.log");
}

function activeRunsPath(): string {
  return join(reeverDir(), "active-runs.json");
}
...
export function installCrashDiagnostics(opts: {
  ...
  // Split prior entries into still-running sessions (other live Reever instances)
