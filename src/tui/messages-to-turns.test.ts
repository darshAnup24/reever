...
describe("messagesToTurns", () => {
  ...
  it("rebuilds a turn with tool call and result", () => {
    const messages: Message[] = [
      ...
      {
        role: "tool",
        content: [{ type: "toolResult", toolCallId: "tc1", toolName: "read", output: '{"name":"reever"}', isError: false }],
      },
      {
        role: "assistant",
        content: [{ type: "text", text: "It defines the reever package." }],
      },
    ];
    ...
    expect(turns[0]?.assistantText).toBe("It defines the reever package.");
  ...
  it("accumulates multiple tools and assistant text across loop rounds", () => {
    const messages: Message[] = [
      ...
      {
        role: "tool",
        content: [{ type: "toolResult", toolCallId: "tc2", toolName: "read", output: "# Reever", isError: false }],
      },
      {
        role: "assistant",
    ...
    expect(turns[0]?.tools[1]).toMatchObject({ id: "tc2", name: "read", status: "done", output: "# Reever" });
  });
