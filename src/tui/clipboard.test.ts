...
describe("clipboard", () => {
  ...
  it("falls back to a temp file when clipboard helpers fail", async () => {
    const result = await copyToClipboard("saved locally", {
      ...
      writeTempFile: (text) => `/tmp/reever-clip-test-${text.length}.txt`,
    });
    expect(result.ok).toBe(true);
    expect(result.method).toBe("file");
    expect(result.path).toBe("/tmp/reever-clip-test-13.txt");
    expect(formatCopyStatus(result)).toContain("Copied to /tmp/reever-clip-test-13.txt");
  });
