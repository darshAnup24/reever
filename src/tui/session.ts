...
import type { ReeverRatelBundle } from "../ratel/catalog.js";
import { reloadReeverTooling } from "../ratel/session.js";
...
export interface TuiSessionConfig {
  ...
  /** Repo root where Reever was launched (host tree). */
  hostCwd: string;
...
export async function runTuiSession(config: TuiSessionConfig): Promise<AgentContext> {
  ...
  // Crash breadcrumbs: log JS-level faults to ~/.reever/crash.log and detect a
  ...
  const onDeleteSession = (sessionId: string): { ok: boolean; message: string } => {
    ...
    // leaves no orphaned snapshots growing under ~/.reever/checkpoints.
  ...
  const applyMcpLoad = async (): Promise<McpReloadResult> => {
    ...
    if (ratelBundle) {
      const tooling = await reloadReeverTooling(config.ctx.cwd, activeSessionId);
  ...
  const configureProvider = (
    ...
    if (activate) {
      ...
      controller.setStatusHint(`${display} configured — saved to ~/.reever/config.json`);
    }
  };
  ...
  try {
    renderer = await createCliRenderer({
      ...
      // OpenTUI destroys itself for external exit signals. Resolve Reever's own
    ...
    if (!config.meta.faux && config.meta.providerConfigured === false) {
      ...
      controller.setStatusHint(
        "Previous Reever session ended unexpectedly — details in ~/.reever/crash.log",
      );
    ...
    // Reclaim old shadow-git checkpoint repos under ~/.reever/checkpoints (gc the
