...
// Isolate config reads from the developer's real ~/.reever/config.json so
...
beforeEach(() => {
  ...
  configHome = mkdtempSync(join(tmpdir(), "reever-commands-test-"));
...
describe("processCommand", () => {
  ...
  it("runs a project custom command with arguments", () => {
    const project = mkdtempSync(join(tmpdir(), "reever-custom-command-"));
    try {
      mkdirSync(join(project, ".git"));
      mkdirSync(join(project, ".reever", "commands"), { recursive: true });
      writeFileSync(
        join(project, ".reever", "commands", "review.md"),
        "---\ndescription: Review code\n---\nReview {{args}} carefully.",
      );
      expect(processCommand("/review src/main.ts", { ...ctx, liveCwd: project })).toEqual({
        type: "prompt",
        prompt: "Review src/main.ts carefully.",
      });
    } finally {
  ...
  it("shows active instruction files in precedence order", () => {
    const project = mkdtempSync(join(tmpdir(), "reever-instructions-"));
    try {
      mkdirSync(join(project, ".git"));
      writeFileSync(join(project, "AGENTS.md"), "Always run focused tests.");
      const result = processCommand("/instructions", { ...ctx, liveCwd: project });
      expect(result).toMatchObject({ type: "info" });
      if (result.type === "info") {
        expect(result.message).toContain(join(project, "AGENTS.md"));
        expect(result.message).toContain("Always run focused tests.");
      }
    } finally {
      rmSync(project, { recursive: true, force: true });
    }
  ...
  describe("/settings", () => {
    ...
    it("shows workspace overview", () => {
      const r = processCommand("/settings workspace", {
        ...
        liveSessionBranch: "reever/session-abc",
