...
const USAGE = "Usage: reever mcp <list [--live] | auth <server> [--code <code>] | debug <server>>";
...
async function runMcpDebug(args: string[]): Promise<void> {
  ...
  if (!serverName || serverName.startsWith("-")) {
    console.error("Usage: reever mcp debug <server>");
    process.exit(1);
  }
...
async function runMcpAuth(args: string[]): Promise<void> {
  const serverName = args[0];
  if (!serverName || serverName.startsWith("-")) {
    console.error("Usage: reever mcp auth <server> [--code <authorization-code>]");
    process.exit(1);
  }
