...
describe("runMcpCli", () => {
  ...
  describe("list", () => {
    ...
    it("prints config warnings to stderr", async () => {
      vi.mocked(loadMcpConfig).mockReturnValue({
        config: { servers: {} },
        scopes: {},
        warnings: ["Failed to parse ~/.reever/mcp.json"],
      });

      await runMcpCli(["list"]);

      expect(warnSpy).toHaveBeenCalledWith("Failed to parse ~/.reever/mcp.json");
    });
  ...
  describe("debug", () => {
    it("exits with usage when server name is missing", async () => {
      ...
      expect(errorSpy).toHaveBeenCalledWith("Usage: reever mcp debug <server>");
      expect(exitSpy).toHaveBeenCalledWith(1);
    });
