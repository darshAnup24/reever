...
describe("createOtelSpanConsumer", () => {
  ...
  it("exports a per-turn trace root with paired LLM and tool spans", async () => {
    ...
    expect(tool.attributes["reever.tool.ok"]).toBe(true);
    expect(tool.attributes["reever.tool.output_bytes"]).toBe(4);
  ...
  it("keeps the same session.id across turns in one Reever session", async () => {
    const { exporter, consumer } = harness();
  ...
  it("marks tool spans ERROR on failure", async () => {
    ...
    expect(tool.attributes["reever.tool.ok"]).toBe(false);
  ...
  it("pairs parallel tool spans by call id without collision", async () => {
    ...
    expect(read.attributes["reever.tool.output_bytes"]).toBe(2);
    expect(bash.attributes["gen_ai.tool.call.id"]).toBe("b");
    expect(bash.attributes["reever.tool.output_bytes"]).toBe(1);
  });
  ...
  it("nests a subagent's LLM and tool spans under the task tool span", async () => {
    ...
    expect(subagent.attributes["reever.subagent.isolation"]).toBe("shared");
    expect(subagent.attributes["gen_ai.request.model"]).toBe("faux:test");
    expect(subagent.attributes["reever.subagent.turns"]).toBe(3);
  });
  ...
  it("names the trace root from the turn's first user message", async () => {
    ...
    const root = exporter.getFinishedSpans().find((s) => s.attributes["reever.turn.id"] === "t1")!;
    expect(root.name).toBe("refactor the loop");
    expect(root.attributes["reever.turn.id"]).toBe("t1");
  });
...
describe("resolveOtelUserId", () => {
  ...
  beforeEach(() => {
    ...
    home = mkdtempSync(join(tmpdir(), "reever-otel-user-"));
  ...
  it("generates and persists a userId on first OTLP-enabled run", async () => {
    ...
    const raw = readFileSync(join(home, ".reever", "config.json"), "utf8");
