...
export interface SessionCostSummary {
  ...
  /**
   * Estimated tokens not re-sent thanks to context pruning (tool-result
   * elision / compaction). The baseline is documented in
   * `telemetry/tokens-saved.ts`: chars/3.5 estimates of the pre-prune context
   * minus the post-prune snapshot, so it is proportional, not tokenizer-exact.
   */
  tokensSaved?: number;
