...
export interface ReeverRatelBuildOptions {
  tools: AnyTool[];
  cwd: string;
...
export interface ReeverRatelSession {
  bundle: ReeverRatelBundle;
 ...
 */
export class ReeverRatelBundle {
  readonly toolCatalog: ToolCatalog;
  skillCatalog: SkillCatalog;
  readonly settings: RatelSettings;
  private readonly reeverTools: Map<string, AnyTool>;
  ...
  private constructor(
    ...
    settings: RatelSettings,
    upstreamServers: UpstreamServerInfo[],
    reeverTools: Map<string, AnyTool>,
  ) {
    this.toolCatalog = toolCatalog;
    this.skillCatalog = skillCatalog;
    this.settings = settings;
    this.reeverTools = reeverTools;
  ...
  static async create(
    ...
    const reeverTools = new Map<string, AnyTool>();

    const nativeTools = opts?.tools ?? coreToolsForRatel();
    for (const tool of nativeTools) {
      registerReeverTool(toolCatalog, tool, reeverTools);
    }

    const mcp = await loadMcpIntoRatelCatalog(toolCatalog, cwd);
    for (const tool of mcp.tools) {
      reeverTools.set(tool.name, tool);
    }
    ...
    const bundle = new ReeverRatelBundle(
      toolCatalog,
      skillCatalog,
      settings,
      upstreamServers,
  ...
  static build(opts: ReeverRatelBuildOptions): ReeverRatelBundle {
    const settings = opts.settings ?? resolveRatelSettings();
    ...
    return new ReeverRatelBundle(toolCatalog, skillCatalog, settings, upstreamServers, reeverTools);
  }
  ...
  static buildForChild(childTools: AnyTool[], cwd: string, settings?: RatelSettings): ReeverRatelBundle {
    return ReeverRatelBundle.build({ tools: childTools, cwd, settings: settings ?? resolveRatelSettings() });
  }

  /** Full Reever tool registry — used for approval and hook-aware execution. */
  allReeverTools(): AnyTool[] {
    return [...this.reeverTools.values()];
  }

  getReeverTool(toolId: string): AnyTool | undefined {
    return this.reeverTools.get(toolId);
  }

  catalogSize(): number {
    return this.reeverTools.size;
  }
  ...
  executionTools(): AnyTool[] {
    ...
    for (const tool of this.allReeverTools()) byName.set(tool.name, tool);
  ...
  resolveToolsForTurn(query: string): TurnToolResolution {
    ...
    const telemetry: RatelResolutionSnapshot = {
      catalogSize: this.reeverTools.size,
  ...
  private wrapGateway(
    ...
    return {
      ...
      ...(honorUnderlyingApproval
        ? {
            needsApproval(args: z.infer<typeof invokeToolSchema>, ctx: AgentContext) {
              ...
              const underlying = self.reeverTools.get(toolId);
      ...
      async execute(args, ctx, signal): Promise<ToolResult> {
        ...
        return gatewayResultToReever(raw);
      },
    };
  }
  ...
  private async executeInvokeTool(
    ...
    const reeverTool = this.reeverTools.get(toolId);

    if (reeverTool) {
      const parsed = reeverTool.schema.safeParse(innerArgs);
      if (!parsed.success) {
      ...
      const result = await reeverTool.execute(parsed.data, ctx, signal);
      return result;
    }

    const raw = await this.gatewayInvoke.execute(args);
    return gatewayResultToReever(raw, formatMcpInvokeOutput);
  }
  ...
  private async executeGetSkillContent(
    ...
    const out = gatewayResultToReever(result);
...
function registerReeverTool(
  ...
  catalog.register({
    ...
    execute: async () => {
      throw new Error(`Tool ${tool.name} must run through the Reever agent loop`);
    },
  });
}
...
function gatewayResultToReever(
  result: unknown,
