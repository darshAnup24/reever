...
describe("loadMcpIntoRatelCatalog (mock MCP)", () => {
  ...
  beforeEach(() => {
    home = mkdtempSync(join(tmpdir(), "reever-ratel-mcp-"));
...
describe("ReeverRatelBundle + mock MCP", () => {
  let home: string;
  let cwd: string;
  let prevHome: string | undefined;

  beforeEach(() => {
    home = mkdtempSync(join(tmpdir(), "reever-ratel-bundle-mcp-"));
    cwd = mkdtempSync(join(tmpdir(), "reever-ratel-cwd-"));
    prevHome = process.env.HOME;
    process.env.HOME = home;
    registerMcpServer.mockReset();
  ...
  it("ingests MCP tools into the catalog and pre-filters per turn", async () => {
    const { ReeverRatelBundle } = await import("./catalog.js");
    const session = await ReeverRatelBundle.create(cwd, {
