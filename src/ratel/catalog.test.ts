...
describe("ReeverRatelBundle", () => {
  it("keeps unpinned tools reachable through BM25 and the gateway", () => {
    const bundle = ReeverRatelBundle.build({
      ...
      settings: {
        ...
        topKTools: 5,
        topKSkills: 2,
        pinnedTools: ["read", "edit", "bash", "grep"],
        controlFraction: 0,
      },
    });

    const resolved = bundle.resolveToolsForTurn("find TypeScript files by glob pattern");
    expect(resolved.tools.some((tool) => tool.name === "find")).toBe(true);
    expect(resolved.tools.some((tool) => tool.name === "search_capabilities")).toBe(true);
    expect(resolved.tools.some((tool) => tool.name === "invoke_tool")).toBe(true);
  });
