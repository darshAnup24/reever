...
describe.runIf(bunAvailable())("reever --headless --faux", () => {
  let home: string;
  let work: string;

  beforeAll(() => {
    home = mkdtempSync(join(tmpdir(), "reever-headless-home-"));
    work = mkdtempSync(join(tmpdir(), "reever-headless-work-"));
  });
  ...
  it("exits non-zero when --headless is given without a prompt", () => {
    ...
    expect(result.stderr).toContain("Usage: reever --headless");
  }, 70000);
});
