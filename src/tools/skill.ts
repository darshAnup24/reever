...
const skillWriteSchema = z.object({
  ...
  scope: z
    ...
    .optional()
    .describe(
      "Where to save the skill. global (default): ~/.reever/skills/<name>/SKILL.md — persists across projects. "
      + "project: .reever/skills/<name>/SKILL.md in the repo root — committed with the project.",
    ),
});
...
export const skillWriteTool: Tool<z.infer<typeof skillWriteSchema>> = {
  ...
  async execute({ action, name, description, instructions, version, scope }, ctx: AgentContext) {
    ...
    if (scope === "project") {
      ...
      baseDir = join(root, ".reever", "skills");
    } else {
