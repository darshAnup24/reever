...
describe("readTool", () => {
  ...
  beforeEach(async () => {
    cwd = await mkdtemp(join(tmpdir(), "reever-read-"));
  ...
  it("caps an unbounded read at the default line limit", async () => {
    ...
    expect(result.output).toContain("L500");
    expect(result.output).not.toContain("L501\n");
    expect(result.output).toContain("100 more lines");
  });

  it("caps a single-line file at 64 KiB", async () => {
    await writeTool.execute(
      { path: "minified.js", content: "x".repeat(80 * 1024) },
      ctx,
      new AbortController().signal,
    );
    const result = await readTool.execute(
    ...
    expect(result.output).toContain("output truncated to 65536 bytes");
  });
});
