...
export function resolveConfiguredActiveProvider(): Provider {
  ...
  for (const provider of listProviders()) {
    // Credential-free local backends such as Ollama must be explicitly chosen.
    // Otherwise a missing cloud key would silently redirect requests to a
    // possibly unavailable local daemon and a completely different model.
    if (provider.authStrategy !== "none" && provider.isConfigured()) return provider;
  }
  return requested;
}
