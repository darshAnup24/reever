...
export interface AssistantMessage extends Message {
  role: "assistant";
  model: string;
  /** The provider that actually served this message (the router may fail over). */
  provider?: string;
  usage?: Usage;
  stopReason?: string;
...
export interface StreamAssistantOptions {
  ...
  sessionId?: string;
  /**
   * Target a specific registered provider instead of the active one. Set by the
   * model router when evaluating a chain candidate; the model id must make sense
   * for that provider.
   */
  providerId?: string;
}
...
/** How a provider authenticates. Local providers can require no credentials. */
export type AuthStrategy = "api-key" | "none";
