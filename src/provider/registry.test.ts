...
describe("provider registry", () => {
  ...
  beforeEach(() => {
    ...
    home = mkdtempSync(join(tmpdir(), "reever-registry-test-"));
  ...
  it("registers built-in providers", async () => {
    ...
    expect(listProviders().some((p) => p.id === "ollama")).toBe(true);
    ...
    expect(metadataProviders().some((m) => m.id === "ollama")).toBe(true);
  });
  ...
  it("does not silently activate a credential-free local provider", async () => {
    const { saveConfig, loadConfig } = await import("../config/config.js");
  ...
  it("uses a credential-free local provider when explicitly selected", async () => {
    const { saveConfig } = await import("../config/config.js");
    saveConfig({ provider: { active: "ollama" } });
    const { repairActiveProviderIfNeeded } = await import("./registry.js");
    expect(repairActiveProviderIfNeeded().id).toBe("ollama");
  });
