...
import { getProvider, resolveActiveProvider } from "./registry.js";
...
export const streamAssistant: StreamAssistantFn = async (
  ...
  const provider =
    options.providerId ? (getProvider(options.providerId) ?? resolveActiveProvider()) : resolveActiveProvider();
