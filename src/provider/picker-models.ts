...
const FULL_CATALOG_PICKER_PROVIDERS = new Set(["opencode-zen", "ollama"]);
...
export async function loadPickerModels(providerId?: string): Promise<string[]> {
  ...
  try {
    ...
    // For a local Ollama catalog, only offer models that are actually pulled.
    // Other providers retain curated fallbacks because remote catalogs can be
    // incomplete even when the featured aliases are valid.
    const base = validated.length > 0 || provider.id === "ollama" ? validated : curated;
