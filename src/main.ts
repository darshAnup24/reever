...
import { routeStreamAssistant } from "./provider/router.js";
import { generateSessionId, getLastEventTimestamp, latestSessionForCwd, listSessions, replayLog, replaySessionMeta, resolveStartupSessionId, sessionPath } from "./session/log.js";
...
import { bootstrapReeverTooling } from "./ratel/session.js";
...
async function main(): Promise<void> {
  ...
  const { prompt, useFaux, headless, listSessions: listSessionsFlag, chat, resumeId, continueLatest, worktree, autoAcceptCli, approvalMode } =
    parseCliArgs(argv);
  ...
  if (headless) {
    if (!prompt) {
      console.error("Usage: reever --headless <prompt>");
      process.exit(1);
    }
  ...
  await runInteractive({ initialMessage: prompt || undefined, useFaux, approvalMode, autoAcceptCli, resumeId, continueLatest, worktree });
}
...
function resolveProvider(useFaux: boolean): { provider: StreamAssistantFn; model: string } {
  ...
  if (!active.isConfigured()) {
    console.error(
      ...
      + "Use --faux for an offline demo, set its API key env var, or add it to ~/.reever/config.json.",
    );
    process.exit(1);
  }
  ...
  return { provider: routeStreamAssistant(streamAssistant), model: resolveProviderSlot(active.id, "main") };
}
...
function resolveInteractiveProvider(useFaux: boolean): {
  ...
  const active = repairActiveProviderIfNeeded();
  return {
    provider: routeStreamAssistant(streamAssistant),
...
async function runInteractive(opts: {
  ...
  const resumeId = opts.resumeId
    ?? (opts.continueLatest ? latestSessionForCwd(hostCwd)?.sessionId : undefined);
  ...
  if (resumeId) {
    const path = sessionPath(resumeId);
    ...
    process.stderr.write(
      `Resuming session ${resumeId} — ${turns} turn${turns !== 1 ? "s" : ""}, last active ${ago}\n`,
    );
    sessionId = resumeId;
  } else {
    if (opts.continueLatest) {
      process.stderr.write("No previous session found for this project; starting a new session.\n");
    }
  ...
  const tooling = await bootstrapReeverTooling(sessionMeta?.cwd ?? hostCwd, sessionId);
...
async function runHeadless(opts: {
  ...
  const hooks = createSessionHooks();
  const tooling = await bootstrapReeverTooling(hostCwd, effectiveSessionId);
  for (const warning of tooling.mcpWarnings) console.warn(warning);
...
async function runOneShotMode(opts: { prompt: string; useFaux: boolean }): Promise<void> {
  ...
  if (opts.useFaux) {
    provider = fauxOneShot(
      "Hello from Reever! Phase 1 provider stream is working (faux mode).",
    );
    model = "faux:test";
  } else {
    const requestedId = activeProviderId();
    ...
    if (!active.isConfigured()) {
      console.error(
        `Provider "${active.id}" (${active.displayName}) is not configured. `
        + "Use --faux for an offline demo, or configure it in ~/.reever/config.json.",
      );
      process.exit(1);
    }
    if (active.id !== requestedId) {
  ...
  await runOneShot(
    provider,
    opts.prompt,
    { model, system: "You are Reever, a concise coding assistant." },
