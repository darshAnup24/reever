# Contributing to Reever

Thanks for your interest in improving Reever! This guide covers how to set up your
...
Reever runs on **[Bun](https://bun.sh) ≥ 1.1** — this is required, not optional.
...
- **MCP servers are user config, not code.** `src/mcp/` loads and adapts
  servers declared in `~/.reever/mcp.json` (merged with a project-local
...
- **Skills are `SKILL.md` files, not code.** `src/skills/discovery.ts` scans
  `.reever/skills/`, `.claude/skills/`, and their `~`-rooted equivalents at
