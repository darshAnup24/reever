...
ensure_path_hint() {
  ...
  if [[ ":$PATH:" != *":${bun_bin_dir}:"* ]]; then
    echo
    echo "Add Bun to your PATH so the reever command is available:"
...
begin_step "Building reever"
bun run build >/dev/null 2>&1
finish_step "Building reever"

begin_step "Initializing ~/.reever/config.json"
bun run init-config >/dev/null 2>&1
finish_step "Initializing ~/.reever/config.json"

begin_step "Linking reever command"
bun link >/dev/null 2>&1
finish_step "Linking reever command"
...
echo "Keys are saved to ~/.reever/config.json."
echo
