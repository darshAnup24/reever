...
## 4. What to implement in Reever (issue #241)

Based on this research, the right approach for Reever is:
...
Directory search: project `.reever/skills/` and `.claude/skills/` (cwd-to-root traversal, .reever tier before .claude tier) → `~/.reever/skills/` (global fallback).
...
- **Scope: project skills** — `skill_write({ scope: "project" })` is already supported; add `.reever/skills/` to `.gitignore` or commit it by convention
