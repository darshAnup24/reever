...
> and best practices relevant to Reever's phases. As with the SPEC references, the goal is
...
after a local clone, not a confirmed line reference. All `src/...` references to **Reever**
...
relevant to Reever's Phase 8 subagent work (#36, #37). Where Roo and Kilo agree, the Roo
source is cited because it's the shared implementation.

---

## Pattern map → Reever phases

| # | Pattern (Cline / Kilo) | Reever phase | Where in Reever | Status |
|---|------------------------|-----------|---------------|--------|
...
**Reever already does this** — `runLoop` is headless and emits to a sink (`src/agent/events.ts`),
...
Reever embeddable the way the Cline SDK is — low-cost, high-leverage if headless/CI use is ever wanted.
...
running. This is essentially Reever's Phase 4 approval modes (`normal` / `auto-accept` / `plan`).

**Refinement worth stealing:** Cline shows the diff *as the approval prompt itself* — the
human approves the exact rendered diff, not just "edit `foo.ts`? y/n". Reever renders diffs
...
This is where Cline and Roo have invested the most, and where Reever's `src/edit/replacers.ts`
has clear gaps. Reever today ships the opencode-style matcher chain (`simpleMatch` →
...
1. **Line-number hints / anchors.** Roo's `MultiSearchReplaceDiffStrategy` accepts a
   ...
   exactly the case Reever's exact-unique matcher *rejects* today. Big reliability gain on
   repetitive code.
2. **Order-invariant multi-block apply with a running delta.** Multiple SEARCH/REPLACE
   ...
   the model emitting blocks out of order. Reever's `edit` takes `{ edits: [...] }` but applies
...
3. **Middle-out fuzzy search + Levenshtein-normalized similarity threshold.** When the line
   ...
   under tolerance). Reever has `levenshteinMatch` but runs it as a last-resort whole-block
...
- **Per-model diff format.** Cline ships multiple block delimiters because models differ
  ...
  another. Relevant once Reever grows beyond OpenRouter (the §6 provider issues #41–#46) or
  adds dumb-model support (Phase 8).
- **Diagnostic-rich failure.** On no-match, Roo returns similarity scores + surrounding
  context so the *model* can self-correct on the next turn. Reever's `findMatch` throws a name
  list; returning a structured "closest candidate + score" `isError` would feed Reever's
...
Maps directly onto Reever's Phase 8 **agent presets** (`src/agent/presets.ts` in #36): Reever's
...
- **Kilo's allow/ask/deny glob model** is essentially a richer approval policy; it's a good
  template if Reever's `src/approval/policy.ts` ever needs per-path granularity (and it composes
...
relevant prior art for Reever's subagent/`task` tool. Mechanics (from Roo's tool + docs):
...
- **Validation:** Reever's #36 design matches Kilo on isolation-of-context, summary-return, and
  depth-1 — good signal the plan is sound.
- **Where Reever goes further (and should):** Kilo's parallelism story is weak — subtasks are
  serial and **share the host workspace via isolated *context* only**, not isolated
  *filesystem*. Reever's #37 (sandbox-per-child via E2B) is a genuine improvement: it's what
  lets children run **in parallel** without colliding on the working tree, which Kilo can't
  safely do. So Reever should *not* copy Kilo's "serial + shared tree" model for parallel work —
...
This is a strong, *currently-missing* Reever capability and it's directly relevant to #48
...
scoped to commit workspace state to `~/.reever/checkpoints/<session>` after each mutating tool,
...
Reever's `src/agent/compaction.ts` already does the core of this — `COMPACT_THRESHOLD = 0.85`,
...
1. **Decouple the API view from the on-disk log (non-destructive condense).** Reever persists
...
2. **A manual `/compact` command.** Cline exposes `/smol` (= `/compact`) so users can condense
   ...
   genuinely useful before a big new sub-task. Reever already has the machinery; it just isn't
   user-triggerable.

Reever's **tool-result elision** (`ELIDED_PREFIX`, `>2000` token threshold) is a nice lever
...
so a repo can teach the agent its conventions. This is *exactly* Reever's `before_prompt` hook +
...
mode can be denied MCP entirely. Reever's Phase 11 (#10) already plans to route every MCP tool
...
> *how it's assembled*, on a spectrum from tiny/user-owned (Reever, pi) to large/dynamically
> composed per model (opencode, Cline, Roo/Kilo).

| Agent | Size | Assembly | Stored as | Project rules | Per-model | Per-role |
|-------|------|----------|-----------|---------------|-----------|----------|
| **Reever** (today) | tiny (1 line) | static string | config value (`config.ts:87`) | ❌ none | ❌ | ✅ sub-prompts only |
...
**Where Reever stands.** The most minimal of the six, deliberately: the whole system prompt is
one config string (`src/config/config.ts:87`, *"You are Reever, a coding agent…"*), user-overridable
via `~/.reever/config.json`, passed as `options.system` → `src/provider/stream.ts:90`. Two things
Reever already gets *right*: tool docs stay **out** of the system prompt (they ride the `tools`
...
blocks (Roo's MCP-only-if-present). (2) Keep prompt text out of code in `.txt`/`.md` (Reever does
...
Reever doesn't read its own**. (4) Inject a dynamic env block (cwd/date/model/platform). (5)
Per-model prompt variants matter once you support many providers. (6) Always provide an override
escape hatch (`--system`, `SYSTEM.md`, replace-mode) — Reever's config string covers this.
...
**Design for Reever (#52).** Reever has no todo tool, but the substrate is in place. Recommended:
...
- **VC stance:** out of git by default, with an **optional `.reever/todo.md` export** for a
...
6. **Todo tool (#52)** — `todowrite` (session-scoped, ≤1 `in_progress`), re-injected each turn
   via the same `before_prompt` seam; kept out of git by default with an optional `.reever/todo.md`
