...
> **Taste** learning system and maps its observable patterns to Reever.
...
- the performance claims on the Taste page are marketing claims, not evidence
  suitable for Reever design decisions.

This is therefore a product-pattern study, not a source audit. Reever should
...
Reever already has instructions, skills, and JSONL sessions. It lacks the Taste
...
Reever currently has no reliable “accept” signal, and silence after an edit does
not mean the user endorses the implementation pattern.

## Pattern map

| # | Command Code pattern | Reever today | Recommendation |
|---|---|---|---|
...
| 9 | `/learn-taste` imports prior agent sessions | Reever has replayable JSONL | Add an explicit offline import command later |
...
| 14 | `/memory` edits `AGENTS.md` | Reever discovers nested/global `AGENTS.md` | Reever is already stronger on nested discovery |
...
extraction plus request-time context. Reever should design against that concrete
...
| Signal | Reliability | Safe Reever v1 behavior |
|---|---:|---|
...
Reever should begin with explicit directives and corrections because they are
...
Learning creates durable state from conversation content; an Reever v1 should:
...
algorithm is private. Reever's existing prompt pipeline offers two known options:
...
- add BM25 retrieval using Reever's existing Ratel/catalog patterns;
...
already counted by Reever's compaction budget.
...
Both are better as explicit commands than background startup work. Reever already
has append-only session logs under `~/.reever/sessions/`, so a future importer can:
...
## 7. Memory, skills, and preferences in Reever

Reever's current layers are deliberately different:

| Reever mechanism | Current behavior | Missing piece |
|---|---|---|
...
`skill_write` is already Reever's self-learning mechanism for multi-step
...
## 8. Proposed Reever design: reviewed preference ledger

### Storage

```text
<repo>/.reever/preferences.md
~/.reever/preferences.md
```
...
Reever's in-process hook registry already has broader lifecycle coverage
...
editing. Reever already discovers nested project `AGENTS.md` files plus a global
...
resume list. Reever could tag automation sessions similarly if headless use makes
...
2. **P1 — Explicit session importer.** Scan Reever JSONL for directives and
...
- Reever: [`skills-research.md`](../skills-research.md),
