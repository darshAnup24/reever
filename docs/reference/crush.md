...
> production terminal coding agent — for patterns relevant to Reever. Crush is Go + Bubble
> Tea; Reever is TypeScript + OpenTUI. The goal is to mirror the *approach*, not copy code
...
All `src/...` references to **Reever** are exact.

## Positioning (why Crush matters)

Crush sits in the same product category as Reever — a single-binary terminal agent with
...
several features Reever has not built yet — most notably **LSP integration** and a
...
| # | Pattern (Crush) | Reever phase | Where in Reever | Status |
|---|-----------------|-----------|---------------|--------|
...
| 12 | Auto-background after N seconds on slow foreground cmds | tools | `src/tools/bash.ts` | 💡 Crush defaults 60s; Reever uses explicit `background: true` |
...
Tea UI subscribes via `internal/pubsub`. Tools never touch the terminal. Reever's `runLoop`
...
can route to small. Reever maps this to **main** + **cheap** tiers and per-preset role
...
Reever's `explore` preset but with stricter output rules ("one word answers are best").

**Refinement worth noting:** Crush can switch provider/model mid-session while SQLite
retains full message history. Reever already supports `/model`; ensure compaction summaries
...
Crush's standout differentiator vs Reever and most terminal agents:
...
Reever lists LSP/plan-mode as out of scope in SPEC §1, but Crush demonstrates that even a
...
1. Config block in `~/.reever/config.json` mirroring Crush's `lsp` stanza.
...
(`old_string` / `new_string`), same family as Reever's edit tool — not unified-diff blocks
(Cline-style). Differences from Reever worth stealing:

| Behavior | Crush | Reever |
|----------|-------|------|
...
Reever's hook registry (`src/hooks/registry.ts`) supports more lifecycle events
...
1. **Hook metadata on tool results** — Crush attaches `HookMetadata` so the TUI shows a
   hook indicator (`internal/hooks/hooks.go`). Reever could emit a `hook` field on tool
...
2. **Explicit allow → skip permission** — Crush's `hookedTool` (`internal/agent/hooked_tool.go`)
   ...
   bypassing the interactive prompt. Reever's `approval-gate` hook could mirror this.
...
4. **Sub-agents skip hooks** — Crush never fires hooks inside sub-agents; only the top-level
   `task` invocation is wrapped. Reever should document/enforce the same rule for `task` children.
...
Reever's approval modes (`src/approval/policy.ts`: `normal` / `auto-accept` / `plan`) plus
...
**Action (low-cost):** extend `~/.reever/config.json` with optional
...
Crush's `internal/skills/` implements the Agent Skills standard (same as Reever's
...
Reever's `skill-inject` hook and `/skills` palette are equivalent for discovery/activation.
**Action:** consider builtin `reever-config` skill and skill-activation telemetry in session
...
`auto_background_after` (default 60s), `job_output`, and `job_kill`. Reever implemented the
...
Reever returns a timeout error and tells the model to retry with `background: true`
...
Reever only has the approval bar for binary allow/deny on tool calls. A `question` tool would
...
Reever exposes similar data via `/settings`, MCP palette, and skills palette — but **not** as
a tool the model can invoke. Useful for support flows and for the agent to re-read config
after the user edits `~/.reever/config.json` mid-session.

**Action (low-cost):** `reever_info` tool returning redacted config summary (no API keys).
...
Reever's `prompt-inject` hook loads `AGENTS.md` from repo root. **Gap:** Crush's breadth
means switching from Claude Code / Cursor / Crush to Reever preserves more user investment
...
Reever's `src/ratel/*` module sketches remote/detached operation but there is no equivalent
local socket server for "drive Reever headlessly from another process."
...
Reever uses **append-only JSONL** per session (`~/.reever/sessions/<id>.jsonl`) — simpler,
human-grepable, git-friendly for debugging, and aligned with pi. Trade-offs:

| | Crush (SQLite) | Reever (JSONL) |
|---|----------------|--------------|
...
**Action:** no migration needed — JSONL is a deliberate Reever choice. Borrow Crush's
...
Reever's TUI typically blocks input or rejects sends while the agent is running. Enqueueing
...
Reever's shadow-git checkpoints (`src/checkpoint/*`) cover **mutations** but not read
...
Glamour for markdown. Reever's OpenTUI stack (`src/tui/theme.ts`, `markdown.tsx`) is
analogous. Worth studying for:

- **Compact mode** config flag
- **Diff view** component (`internal/ui/diffview/`) — ensure Reever approval embeds the same
...
Reever hardcodes sensible defaults in `src/config/models.ts` and provider modules. A Catwalk-
style **external catalog** (even if Reever-hosted JSON fetched on demand) would reduce churn
...
5. **#365 — `reever_info` tool** (§10): agent-callable redacted config dump.
...
- Reever: [`SPEC.md`](../../SPEC.md), [`docs/reference/cline-kilo.md`](cline-kilo.md), [`docs/skills-research.md`](../skills-research.md)
