# Ratel Integration Plan — Reever / coding_agent
...
| **Tool management** | Dynamic catalog + unified dispatcher — `ReeverRatelBundle` (Direct SDK, hybrid MCP) |
...
`ratel` is an `ReeverRatelBundle` instance, passed from `bootstrapReeverTooling` → `runLoop`. Every LLM call gets a BM25-filtered tool list (replace mode). The bundle is optional — when `ratel.enabled` is `false`, the full flat list is used.

### Tool registration topology

| Layer | Location | Registered into |
|---|---|---|
| Native core tools | `src/ratel/tools.ts:coreToolsForRatel()` | `ToolCatalog` via `registerReeverTool` |
...
Loaded from `~/.reever/config.json` → `ratel.*`. These are sane defaults. `pinnedTools` is the protected core — gateway tools and the six most-used filesystem tools are never trimmed. The protected core needs a review if tool names change (see gap 1 below).
...
| Area | File | Status |
|---|---|---|
| `ToolCatalog` + native tool registration | `src/ratel/catalog.ts:116–162` | Correct — one catalog, `registerReeverTool` wires both BM25 and dispatch |
...
**Fix**: in `executeSingleTool` (`src/agent/loop.ts:105`), when `registry.get(call.name)` returns `undefined` AND Ratel is active AND the tool _exists_ in `ratel.reeverTools`, emit a `ratel_unavailable_tool_call` metric. This distinguishes "Ratel trimmed it" from "genuine hallucination."

```ts
// Pseudocode — add to executeSingleTool when tool === undefined
if (options.ratel?.getReeverTool(call.name)) {
...
2. In `src/ratel/session.ts:bootstrapReeverTooling`, after resolving settings:
   ```ts
